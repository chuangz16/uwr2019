...
public class TestTemplateProcessor implements DataSourceType{
	...
	public void setUp() throws Exception {
		...
		ConstDataSource constds=EasyMock.createMock(ConstDataSource.class);
		EasyMock.expect(dsc.getConstDataSource()).andReturn(constds);
		DataHolder dataholder =EasyMock.createMock(DataHolder.class);
		EasyMock.expect(constds.getDataHolder("sex")).andReturn(dataholder);
		EasyMock.expect(dataholder.getValue()).andReturn("Female");
		EasyMock.expect(constds.getDataHolder("readme")).andReturn(dataholder);
		EasyMock.expect(dataholder.getValue()).andReturn("5");
		EasyMock.expect(constds.getDataHolder("testexper")).andReturn(dataholder);
		EasyMock.expect(dataholder.getExpr()).andReturn("${num}+${readme}");
		EasyMock.expect(dataholder.fillValue()).andReturn(null);
		EasyMock.expect(dataholder.getValue()).andReturn("5.0");
